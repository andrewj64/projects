...
std::shared_ptr<Crypto> Crypto::cryptoFactory(std::function<void(const uint8_t *data, uint32_t len)> encryptCallback,
                                              std::function<void(const uint8_t *data, uint32_t len)> decryptCallback,
                                              Algorithm algorithm)
{	// Instantiate a Crypto object based on user input
	...
	else if (algorithm == Algorithm::eAES)
	{
		std::shared_ptr<Crypto> pAES(new CryptoAES(encryptCallback, decryptCallback));
		return pAES;
	}
	else
	{
		std::cout << "Algorithm selection is invalid." << std::endl;
		exit(1);
	}	
}
